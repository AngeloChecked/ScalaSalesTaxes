...
trait Roundable05 extends Roundable {
  protected def round(price: Double): Double = RoundRules.roundUpNear05(price)
}

trait Importable5 extends Importable with Roundable05 {
  override def importTax: Double =
    if (imported) round(_price / 100 * 5) else 0.0
}
...
class Good(val name: String,
  ...
  def sumTax(value: Double): Double =
    round(((importTax + tax) * quantity) + (value))

  def sumPrice(value: Double): Double = price + value

  override def toString =
    s"$quantity ${if (imported) "imported " else ""}$name: %.2f".format(price)
...
class Book(name: String, quantity: Int, price: Double, imported: Boolean)
    extends Good(name, quantity, price, imported)
    with NoTaxable
