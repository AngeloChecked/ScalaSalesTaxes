...
import model.classic.{Book, Food, Good, Medical}
...
class GoodTest extends FunSuite {
  ...
  test ("Round up nearest .05") {
    ...
    assert(Utils.roundUpNear05(.1)==.1)
  }


  test("1 item imported and taxed"){
    val good = new Good("bottle of perfume",
      1,
      47.50,
      true)
  ...
  test("1 food"){
    val good = new Food("chocolate bar",
      1,
      0.85,
      false)
  ...
  test("3 food imported"){
    val good = new Food("box of chocolates",
      3,
      11.25,
      true)
  ...
  test( "total 1") {
    assert(Utils.goodsTotalPrice(items1)==42.32)
  }

  test( "Sales Taxes total 1") {
    assert(Utils.goodsSalesTaxesTotal(items1)==1.50)
  }

  val items2 = List(
    new Food("box of chocolates",
      1,
      10.00,
      true),
    new Good("bottle of perfume",
      1,
      47.50,
      true)
  )
  ...
  test( "total 2") {
    assert(Utils.goodsTotalPrice(items3)==98.38)
  }

  test( "Sales Taxes total 2") {
    assert(Utils.goodsSalesTaxesTotal(items3)==7.90)
  }
  ...
  val items3 = List(
    ...
    new Good("bottle of perfume",
      ...
      18.99,
      false),
    new Medical("packet of headache pills",
      1,
      9.75,
      false),
    new Food("box of chocolates",
